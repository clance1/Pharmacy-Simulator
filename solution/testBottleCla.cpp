...
void dispense(Bottle& theBottle, std::string& tempDrugName, int& numPills, DATE& dispDate){
        ...
        if(theBottle.isBottleExpired(dispDate)){
            ...
            std::cout << "is expired as of " << theBottle.getDate();
...
int main(int argc, char** argv)
{
    ...
    DATE initialDate(2019, 1, 23); // EXP: 2019 JAN 23 
    ...
    while(!inStream.eof()){
        ...
        if(checkVal == "STOCK"){
            ...
            inStream >> month;
            inStream >> day;
            DATE inspecDate(year, month, day);
            
            std::cout << "INSPECTION on " << inspecDate << std::endl;
